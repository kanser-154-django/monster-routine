...
// NETWORK-FIRST: Always fetches fresh content. No manual version bumping needed.

self.addEventListener('install', e => { self.skipWaiting(); });

self.addEventListener('activate', e => {
  // Clear ALL caches on every activation so old versions never persist
  e.waitUntil(caches.keys().then(keys => Promise.all(keys.map(k => caches.delete(k)))));
  self.clients.claim();
});

self.addEventListener('fetch', e => {
  // Always try network first — never serve stale cache
  e.respondWith(
    fetch(e.request, { cache: 'no-store' }).catch(() => {
      if (e.request.mode === 'navigate') {
        return new Response('<h2 style="font-family:sans-serif;padding:40px;">Offline — please reconnect</h2>', {
...
function scheduleFromClient(data){
  ...
  alarms.forEach(alarm=>{
    ...
    if(msUntil>0&&msUntil<86400000){
      ...
      const id=setTimeout(()=>{
        self.registration.showNotification(msg.title,{body:msg.body,icon:'./icon-192.png',badge:'./icon-192.png',tag:'monster-'+alarm.id,renotify:true,requireInteraction:alarm.id==='wake'||alarm.id==='gym',actions:[{action:'open',title:'Open App'},{action:'dismiss',title:'Dismiss'}]});
      },msUntil);
